app.controller('AuditarProcesosCtrl', ["$scope", "$rootScope", "ProcesosService", "ProtocoloService", "$builder", "$validator",
  function ($scope, $rootScope, ProcesosService, ProtocoloService, $builder, $validator) {
    ...
    vm.OpenURL = function (url, type) {
      const blob = b64toBlob(url.split("base64,")[1], type)
      const blobUrl = window.URL.createObjectURL(blob)
      swal({
        title: "¿Quieres abrir este archivo en esta ventana?",
    ...
    function GetProcesos() {
      vm.cargado = false
      ProcesosService.GetProcesosForAuditoria(vm.Estado).then(function (d) {
        console.log(d.data)
