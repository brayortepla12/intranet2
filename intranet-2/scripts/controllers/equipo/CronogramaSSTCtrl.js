app.controller('CronogramaSSTCtrl', ["$scope", "$rootScope", "HojaVidaSSTService", "ServicioService", "SedeService", "EmpresaService", "EncabezadoService", "FrecuenciaService",
    "$filter",
    function ($scope, $rootScope, HojaVidaSSTService, ServicioService, SedeService, EmpresaService, EncabezadoService, FrecuenciaService, $filter) {
        ...
        $scope.DeleteItem = function (i) {
            swal({
                ...
                if (isConfirm) {
                    ...
                    HojaVidaSSTService.DeleteHojaVida(data).then(function (d) {
        ...
        $scope.BuscarHojaVidas = function () {
            if ($scope.SedeId !== "--") {
                $scope.cargado = false;
                HojaVidaSSTService.GetHojaVidaSedeId_Cronograma($scope.SedeId, $scope.ServicioId, $scope.Estado).then(function (d) {
                    console.log(d.data);
                    $scope.simpleTableOptions = {
                        data: [],
                        aoColumns: [
                            {mData: 'NumeroExtintor'},
                            {mData: 'Ubicacion'},
                            {mData: 'Servicio'},
                            {mData: 'Sector'},
                            {mData: 'ClaseExtintor'},
        ...
        $scope.GenerarExcel = function () {
            HojaVidaSSTService.GenerarExcel($scope.SedeId, $scope.ServicioId, $scope.Estado);
        };
        //</editor-fold>
        ...
        function SetFormat(lst) {
            for (var i in lst) {
                if (lst[i].Estado != 'Inactivo') {
                    lst[i].Opciones = '<a class="btn btn-success btn-xs icon-only white" href="/Polivalente#/sst/ficha_tecnica_3/' + lst[i].HojaVidaId + '" target="_blank"' +
