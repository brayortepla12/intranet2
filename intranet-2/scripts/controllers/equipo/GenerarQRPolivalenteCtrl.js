app.controller('GenerarQRPolivalente', ["$scope", "$rootScope", "HojaVidaService", "ServicioService", "SedeService", "EmpresaService", "EncabezadoService", "FrecuenciaService",
    "$filter",
    function ($scope, $rootScope, HojaVidaService, ServicioService, SedeService, EmpresaService, EncabezadoService, FrecuenciaService, $filter) {
        ...
        $scope.BuscarHojaVidas = function () {
            if ($scope.SedeId != "--") {
                $scope.cargado = false;
                HojaVidaService.GetHojaVidaServicioId($scope.ServicioId).then(function (d) {
                    console.log(d.data);
