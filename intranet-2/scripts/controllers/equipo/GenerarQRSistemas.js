app.controller('GenerarQRSistemas', ["$scope", "$rootScope", "HojaVidaSistemaService", "ServicioService", "SedeService", "EmpresaService", "EncabezadoService", "FrecuenciaService",
    "$filter",
    function ($scope, $rootScope, HojaVidaSistemaService, ServicioService, SedeService, EmpresaService, EncabezadoService, FrecuenciaService, $filter) {
        ...
        $scope.BuscarHojaVidas = function () {
            if ($scope.SedeId != "--") {
                $scope.cargado = false;
                HojaVidaSistemaService.GetHojaVidaServicioId($scope.ServicioId).then(function (d) {
                    console.log(d.data);
