app.controller('ListadoAmbulanciasCtrl', ["$scope", "$rootScope", "HojaVidaAmbulanciaService", "ServicioService", "SedeService", "EmpresaService", "EncabezadoService", "FrecuenciaService",
    "$filter",
    function ($scope, $rootScope, HojaVidaAmbulanciaService, ServicioService, SedeService, EmpresaService, EncabezadoService, FrecuenciaService, $filter) {
        ...
        $scope.DeleteItem = function (i) {
            swal({
                ...
                if (isConfirm) {
                    ...
                    HojaVidaAmbulanciaService.DeleteHojaVida(data).then(function (d) {
        ...
        $scope.BuscarHojaVidas = function () {
            if ($scope.SedeId !== "--") {
                $scope.cargado = false;
                HojaVidaAmbulanciaService.GetHojaVidaSedeId($scope.SedeId, $scope.ServicioId, $scope.Estado).then(function (d) {
                    console.log(d.data);
                    $scope.simpleTableOptions = {
                        data: [],
                        aoColumns: [
        ...
        $scope.ViewItem = function (i) {
            HojaVidaAmbulanciaService.GetHojaVidaHojaVidaId($scope.simpleTableOptions.data[i].HojaVidaId).then(function (d) {
        ...
        function SetFormat(lst) {
            for (var i in lst) {
                if (lst[i].Estado != 'Inactivo') {
                    lst[i].Opciones = '<a class="btn btn-success btn-xs icon-only white" href="/Polivalente#/ambulancia/crear_hoja_vida/' + lst[i].HojaVidaId + '" target="_blank"' +
