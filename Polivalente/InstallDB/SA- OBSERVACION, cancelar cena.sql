...
ADD COLUMN `EDAD` VARCHAR(45) NULL AFTER `ModifiedAt`;
...
DELIMITER $$
USE `polivalente`$$
CREATE DEFINER=`ospino`@`%` FUNCTION `ContarComidasPorHDPorDia`(_sector varchar(50), _dia int, _mes int, _annio int) RETURNS int(11)
    DETERMINISTIC
BEGIN 
  DECLARE dist INT;
...
DELIMITER $$
USE `polivalente`$$
CREATE DEFINER=`ospino`@`%` FUNCTION `ContarComidasPorHDPorMes`(_sector varchar(50), _mes int, _annio int) RETURNS int(11)
    DETERMINISTIC
BEGIN 
  DECLARE dist INT;
	SET dist = (SELECT sum(T.TD) + sum(T.TMM) + sum(T.TA) + sum(T.TMT) + sum(T.TC) + sum(T.TMN) FROM (
