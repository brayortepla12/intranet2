...
WHERE 
	PersonaId=2275 and MONTH(fecha) = 11 and YEAR(fecha) = 2019
ORDER BY Dia;
...
ADDTIME(ADDTIME(ifnull(timediff(tabla.S2, tabla.E1), '00:00:00') , ifnull(timediff(tabla.S4, tabla.E3), '00:00:00') ), ADDTIME(ifnull(timediff(tabla.S6, tabla.E5), '00:00:00') , ifnull(timediff(tabla.S8, tabla.E7), '00:00:00' ))) as Total
 FROM (select (ELT(WEEKDAY(DATE_FORMAT(t1.fecha, '%Y%m%d')) + 1, 'Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab', 'Dom')) AS DIA_SEMANA,
        ...
        max(t0.E1_P) E1_P, max(t0.E1_PS) E1_PS, GetDispositivoByControlId(max(E1_C)) E1_C, VerificarEstadoEntrada(2275, SEC_TO_TIME(sum(t0.E1)), t0.fecha) as E1_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.S2)),1,8) S2,
        max(t0.S2_P) S2_P, max(t0.S2_PS) S2_PS, GetDispositivoByControlId(max(S2_C)) S2_C, VerificarEstadoSalida(2275, SEC_TO_TIME(sum(t0.S2)), t0.fecha) as S2_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.E3)),1,8) E3,
        max(t0.E3_P) E3_P, max(t0.E3_PS) E3_PS, GetDispositivoByControlId(max(E3_C)) E3_C, VerificarEstadoEntrada(2275, SEC_TO_TIME(sum(t0.E3)), t0.fecha) as E3_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.S4)),1,8) S4,
        max(t0.S4_P) S4_P, max(t0.S4_PS) S4_PS, GetDispositivoByControlId(max(S4_C)) S4_C, VerificarEstadoSalida(2275, SEC_TO_TIME(sum(t0.S4)), t0.fecha) as S4_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.E5)),1,8) E5,
        max(t0.E5_P) E5_P, max(t0.E5_PS) E5_PS, GetDispositivoByControlId(max(E5_C)) E5_C, VerificarEstadoEntrada(2275, SEC_TO_TIME(sum(t0.E5)), t0.fecha) as E5_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.S6)),1,8) S6,
        max(t0.S6_P) S6_P, max(t0.S6_PS) S6_PS, GetDispositivoByControlId(max(S6_C)) S6_C, VerificarEstadoSalida(2275, SEC_TO_TIME(sum(t0.S6)), t0.fecha) as S6_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.E7)),1,8) E7,
        max(t0.E7_P) E7_P, max(t0.E7_PS) E7_PS, GetDispositivoByControlId(max(E7_C)) E7_C, VerificarEstadoEntrada(2275, SEC_TO_TIME(sum(t0.E7)), t0.fecha) as E7_E,
		SUBSTRING(SEC_TO_TIME(sum(t0.S8)),1,8) S8,
        max(t0.S8_P) S8_P, max(t0.S8_PS) S8_PS, GetDispositivoByControlId(max(S8_C)) S8_C, VerificarEstadoSalida(2275, SEC_TO_TIME(sum(t0.S8)), t0.fecha) as S8_E
        from ct_reporte1 t0
        right join ct_mes t1 on t0.fecha = t1.fecha
        where t1.mes=11 and annio=2019
